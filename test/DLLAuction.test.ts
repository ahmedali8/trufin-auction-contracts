...
import type { DLLAuction, MockToken } from "../types";
import { mockTokenFixture } from "./shared/fixtures";

describe("DLLAuction Tests", function () {
  // signers
  ...
  // contracts
  let auctionContract: DLLAuction;
  ...
  beforeEach(async function () {
    const loadFix = async () => {
      ...
      const Factory = await ethers.getContractFactory("DLLAuction");
  ...
  describe("Auction Initialization", function () {
    ...
    it("Should not start an auction if another is in progress", async function () {
      await auctionContract.startAuction(parseEther("100"), 3600);
      await expect(
        auctionContract.startAuction(parseEther("50"), 3600)
      ).to.be.revertedWithCustomError(auctionContract, "AuctionAlreadyInProgress");
    });
  });
  ...
  describe("Bidding Mechanism", function () {
    ...
    it("Should reject bids with incorrect ETH amount", async function () {
      ...
      ).to.be.revertedWithCustomError(auctionContract, "IncorrectETHSent");
    });

    it("Should reject duplicate bids from the same bidder", async function () {
      ...
      await expect(
        auctionContract
          .connect(alice)
          .placeBid(parseEther("5"), parseEther("1.2"), { value: parseEther("6") })
      ).to.be.revertedWithCustomError(auctionContract, "BidAlreadyPlaced");
    });
  });
  ...
  describe("Auction Finalization", function () {
    beforeEach(async function () {
      ...
      console.log("lastBidder: ", await auctionContract.lastBidder());

      expect(await auctionContract.topBidder()).to.equal(bob.address);
    ...
    it("Should reject finalization before auction ends", async function () {
      await auctionContract.startAuction(parseEther("100"), 3600);
      await expect(auctionContract.finalizeAuction()).to.be.revertedWithCustomError(
        auctionContract,
        "AuctionEnded"
      );
    });
